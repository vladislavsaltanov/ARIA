...
Раздел «Журнал» в настройках: тумблер «вести журнал» (по умолчанию ВЫКЛ, пишется в settings.json, применяется живьём), селектор уровня, путь к файлу, кнопка «Показать файл», пометка про приоритет `ARIA_LOG_LEVEL`. Стартовый уровень: env (если задан, включает журнал) иначе настройка. Старые settings.json без ключа молча остаются без журнала.
